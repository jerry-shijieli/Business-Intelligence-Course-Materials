...
def main():
    ...
    counts = stream(ssc, pwords, nwords, 600)
    save_for_plot(counts)


def save_for_plot(counts):
    """
    ...
    fout.write("Time,Positive Count,Negative Count\n")
    for index in range(len(indexCount)):
        fout.write(str(indexCount[index])+','+str(posCount[index])+','+str(negCount[index])+'\n')
    fout.close()
