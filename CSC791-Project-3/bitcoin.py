...
def computeDelta(wt, X, Xi):
    ...
    """
    topSum = 0 # top sum of equ(6)
    bottomSum = 0 # bottom sum of equ(6)
    N = Xi.index # # of data points in training set
    XVect = (X[:-1]) # vector of X attributes
    XMean = np.mean(XVect)
    XStd = np.std(XVect)
    for i in xrange(len(N)):
        XiVect = Xi.iloc[i][:-1] # vector of Xi attributes
        XiMean = np.mean(XiVect)
        XiStd = np.std(XiVect)
        sTop = sum((XVect-XMean) * (XiVect-XiMean)) # top sum of equ(9)
        M = X.size - 1;
        sBottom = (M * XStd * XiStd) # bottom sum of equ(9)
        Yi = Xi.iloc[i][-1] # label of the data point
        similarity = wt * sTop / sBottom # value of equ(9) for data index i
        topSum += Yi * math.exp(similarity)
        bottomSum += math.exp(similarity)
