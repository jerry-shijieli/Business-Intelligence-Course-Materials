...
def similarity(a,b):
  mean_a = a.mean()
  mean_b = b.mean()
  #std_a = (np.std(a,dtype = np.float64))**2
  std_a = np.std(a,dtype = np.float64)
  #std_b = (np.std(b,dtype = np.float64))**2
  std_b = np.std(b,dtype = np.float64)
  a = a - mean_a 
  b = b - mean_b
  numerator = sum(a*b)
  M = a.shape[0]
  denominator = M * std_a * std_b
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(length):
      trainX = np.append(trainX,similarity(X[:(M-1)],Xi.iloc[i][:(M-1)]))
    # YOUR CODE GOES HERE
    
    trainX = trainX * wt
    trainX = np.exp(trainX)
    trainLabel = np.asarray(Xi[['Yi']])
    trainLabel = np.reshape(trainLabel,-1)

    return sum(trainX*trainLabel)/sum(trainX)
...
for i in xrange(0,len(train1_90.index)) :
  # train2_90.iloc return the i row in the train2_90
