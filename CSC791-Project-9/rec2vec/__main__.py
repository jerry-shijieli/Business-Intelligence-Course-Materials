...
def process(args):
    ...
    # YOUR CODE HERE
    walks = graph.build_deepwalk_corpus(G, args.number_walks, args.walk_length+1)
    ...
    print "RMSE = %f" % mean_squared_error(tr, pr)
...
def predict_rating(model, nodedict, user, movie):
    ...
    for rating in range(1,6):
        similarity = model.similarity(nodedict[user].id, nodedict[movie+'_%s'+str(rating)].id)
