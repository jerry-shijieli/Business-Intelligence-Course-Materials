...
def records_to_graph():
    ...
    users = ratings.keys()
    for userID in users:
        node = Node(index, userID, 'user')
        nodelist.append(node)
        nodedict[userID] = node
        index += 1
    for m_id in movies:
        node = Node(index, movies[m_id].name, 'movie')
        nodelist.append(node)
        nodedict[m_id] = node
        index += 1
    for director in directors:
        node = Node(index, director, 'director')
        nodelist.append(node)
        nodedict[director] = node
        index += 1
    for actor in actors:
        node = Node(index, actor, 'actor')
        nodelist.append(node)
        nodedict[actor] = node
        index += 1
    for genre in genres:
        node = Node(index, genre, 'genre')
        nodelist.append(node)
        nodedict[genre] = node
        index += 1
    for m_id in movies:
        for rating in range(1, 6):
            node = Node(index, m_id + '_%s' + str(rating), 'rating')
            nodelist.append(node)
            nodedict[m_id + '_%s' + str(rating)] = node
            index += 1
    ...
    for userID in ratings.keys():
        for m_id in ratings[userID]:
            rating = ratings[userID][m_id]
            nodedict[m_id + '_%s' + str(rating)].neighbors.append(nodedict[userID])  # user to rating
            nodedict[userID].neighbors.append(nodedict[m_id + '_%s' + str(rating)])
            nodedict[m_id + '_%s' + str(rating)].neighbors.append(nodedict[m_id])  # movie to rating
            nodedict[m_id].neighbors.append(nodedict[m_id + '_%s' + str(rating)])
    for m_id in movies.keys():
        director = movies[m_id].director
        if director != None:
            nodedict[m_id].neighbors.append(nodedict[director])  # movie to director
            nodedict[director].neighbors.append(nodedict[m_id])
        actors = movies[m_id].actors
        if len(actors) > 0:
            for actor in actors:
                nodedict[actor].neighbors.append(nodedict[m_id])  # movie to actor
                nodedict[m_id].neighbors.append(nodedict[actor])
        genres = movies[m_id].genres
        if len(genres) > 0:
            for genre in genres:
                nodedict[genre].neighbors.append(nodedict[m_id])  # movie to genre
                nodedict[m_id].neighbors.append(nodedict[genre])
